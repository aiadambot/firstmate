...
Local secondmates may coordinate `local-only` work under `secondmate-provisioning`; its landing remains primary-Firstmate-owned.
...
Use `secondmate-provisioning` for local-only routing and parent landing; remote local-only placement remains unsupported.
