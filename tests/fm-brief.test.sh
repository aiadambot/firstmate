...
test_faster_paths_use_configured_authority_without_stacked_review() {
  ...
  brief="$home/data/$id/brief.md"
  assert_grep "The configured merge authority approves the ready branch, then the primary Firstmate lands it through \`bin/fm-merge-local.sh\`; a secondmate clone" "$brief" \
    "local-only brief lost configured merge authority and guarded landing"
  assert_grep "bin/fm-local-ready.sh $id" "$brief" \
    'local-only brief lost immutable readiness for a seeded secondmate'
