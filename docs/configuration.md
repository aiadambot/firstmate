...
Local secondmate delivery retains ready artifacts and parent landing receipts owned by [`fm-local-ready.sh`](../bin/fm-local-ready.sh) and [`fm-local-delivery-lib.sh`](../bin/fm-local-delivery-lib.sh); parent-owned validation claims and run associations are owned by [`fm-validation-coordinate.sh`](../bin/fm-validation-coordinate.sh).
...
Local secondmate routes also support `local-only` projects through the [secondmate provisioning contract](../.agents/skills/secondmate-provisioning/SKILL.md); their immutable ready heads land only in the parent's canonical project through `fm-merge-local.sh`.
