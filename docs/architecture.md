...
`fm-home-seed.sh` provisions a local isolated home, clones the listed projects into it, initializes newly cloned `no-mistakes` projects, copies the charter to `data/charter.md`, and `fm-spawn.sh --secondmate` launches it through the same session-provider and status-file path as any direct report.
...
`local-only` projects may also be coordinated by a local secondmate under the [`secondmate-provisioning` skill](../.agents/skills/secondmate-provisioning/SKILL.md)'s local delivery contract: landing still merges into the parent home's canonical local checkout through the primary firstmate, and remote placement remains unsupported.
...
`bin/fm-project-mode.sh` remains the one registry parser for the mechanical consumers that have no task in hand: fleet sync's `local-only` skip, backlog handoff's local-only route validation, and home seeding's local-only source binding and no-mistakes initialization.
